...
export async function getHotels(
  ...
  try {
    ...
    const response = await makeBookingComRequest<any>(hotelsUrl);
...
export function getHotelDetails(hotel: any) {
  // Format price with currency
  const name = hotel.hotel_name;
  const price = `${
    hotel.composite_price_breakdown.gross_amount_per_night.currency
  } ${hotel.composite_price_breakdown.gross_amount_per_night.value.toLocaleString()}`;
