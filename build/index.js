...
server.tool("search-flights", "search for flights", {
    ...
    const flights = (await searchFlights(bestFromLocationId, bestToLocationId, sort, cabinClass, departDate, returnDate)) ?? [];
...
server.tool("search-hotels-by-coordinates", "search for hotels by geographic coordinates", {
...
}, async ({ latitude, longitude, arrivalDate, departureDate, radius, adults, childrenAge, roomQuantity, priceMin, priceMax, currencyCode, }) => {
    const hotels = await getHotels(latitude, longitude, arrivalDate, departureDate, radius, adults, childrenAge, roomQuantity, priceMin, priceMax, currencyCode);
    ...
    const hotelDetails = hotels.map((hotel) => getHotelDetails(hotel));
